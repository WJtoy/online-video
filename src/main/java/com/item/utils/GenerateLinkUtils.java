...
public class GenerateLinkUtils {
	...
	public static String generateActivateLink(User user) {
		...
		return "http://localhost:8080/ssm-crud/email/activate?id="+user.getId()+"&"+CHECK_CODE+"="+generateCheckcode(user);
	}
	...
	private static String bytes2Hex(byte[] byteArray) {
		...
		System.out.println(strBuf.toString()+"1111111111");
		return strBuf.toString();
	}
 
	public static boolean verifyCheckcode(User user, String checkCode) {
		
		boolean flag = generateCheckcode(user).equals("df1fb4b26a4ca70af1005326325a590f");
		
		System.out.println(generateCheckcode(user)+"==================="+checkCode);
		return flag;
	}
